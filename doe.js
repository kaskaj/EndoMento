...
const PARAM_RANGES = {
  ...
  brushLengthBase: { min: 0.1, max: 20.0, step: 0.1 },
  brushLengthRange: { min: 1.0, max: 10.0, step: 0.1 },
  brushLengthTopMultiplier: { min: 0.5, max: 2.0, step: 0.05 },
  brushVibration: { min: 0, max: 5.0, step: 0.1 },
...
const BRUSH_LENGTH_NOISE_SCALE = 0.005;
...
function sampleParams(focus, spread) {
  ...
  params.brushLengthBase = Math.max(0, params.brushLengthBase);
  params.brushLengthRange = Math.max(1, params.brushLengthRange);
  return params;
}
...
async function renderCard(card, params) {
  ...
  try {
    ...
    const lengthField = (w, h, scale) => {
      const arr = Array.from({ length: w }, () => Array.from({ length: h }, () => 0));
      for (let x = 0; x < w; x++) {
        for (let y = 0; y < h; y++) {
          arr[x][y] = g.constrain(g.noise(scale * x, scale * y, 0), 0, 1);
        }
      }
      return arr;
    };
    ...
    const brushLengthField = lengthField(g.width, g.height, BRUSH_LENGTH_NOISE_SCALE);

    g.strokeWeight(params.brushWeight);
    const baseLenParam = Math.max(0, params.brushLengthBase ?? 1.5);
    const rangeParam = Math.max(1, params.brushLengthRange ?? 2);
    const lengthMin = baseLenParam / rangeParam;
    const lengthMax = baseLenParam * rangeParam;
    ...
    for (let y = 0; y < params.tiles; y++) {
      for (let x = 0; x < params.tiles; x++) {
        ...
        const lenNoise = brushLengthField[bx]?.[by] ?? 0;
        const baseLen = Math.max(0, g.map(lenNoise, 0, 1, lengthMin, lengthMax)) * tileWidth;
        ...
        if (bottomVal > 0) {
          ...
          const dx = g.cos(ang) * baseLen;
          const dy = g.sin(ang) * baseLen;
        ...
        if (topVal > 0) {
          ...
          const lenTop = baseLen * params.brushLengthTopMultiplier;
